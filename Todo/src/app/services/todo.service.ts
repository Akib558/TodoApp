...
// import { TodoLabels } from '../models/todolabels.model';
...
export class TodoService {
  ...
  getAllTodos(): Observable<TodoLabels[]> {
      ...
      map((response) => {
        if (response.status === 'Success') {
          ...
          return todos.map((todo) => ({
            ...
            labels: {
              labelsarray: todo.labels.split(','),
              mylabelsarray: todo.myLabels.split(','),
            },
          }));
        } else {
  ...
  addTodo(newTodo: Todo): Observable<Todo> {
    return this.http
      .post<ResponseClass>(`${this.baseApiUrl}/api/todoall`, newTodo)
      .pipe(
        map((response) => {
          if (response.status === 'Success') {
  ...
  updateTodoCompletion(todo: Todo): Observable<Todo> {
    ...
    return this.http
      .put<ResponseClass>(`${this.baseApiUrl}/api/todoall/${todo.id}`, todo)
      .pipe(
        map((response) => {
  ...
  deleteTodo(todoId: number): Observable<void> {
    return this.http
      .delete<ResponseClass>(`${this.baseApiUrl}/api/todoall/${todoId}`)
      .pipe(
        map((response) => {
          if (response.status === 'Success') {
            return;
